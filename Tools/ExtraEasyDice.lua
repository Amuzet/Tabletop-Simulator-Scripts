...
function onChat(m,p)if self.getName()=='Easy Dice'and(m:find('^R ')~=nil)then local g=m:gsub('^R ','')local t,s=RF(self,p.color,g)if s>0 then printToColor(p..' only: '..text,p,self.getColorTint())else printToAll(t,self.getColorTint())end end end
