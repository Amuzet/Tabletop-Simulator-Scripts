...
local DeckSites = {
  ...
  tappedout=function(a)printToAll('Tappedout Alters Unsupported',{0.1,0.5,0.8})return a..'?fmt=txt',spawnDeck end,
  ...
  mtggoldfish=function(a)
    ...
    return a:gsub('/deck/','/deck/download/'):gsub('#%w+',''),spawnDeck end end,
...
[i]small,normal,large,art_crop,border_crop[/i] ]]
  ...
  if version < tonumber(v) then
    Usage = Usage..'\n[77ff00]Update:'..tonumber(v)..' Ready on Workshop\n'..wr.url
...
Button = setmetatable({
    ...
    rotation = {0,0,90},
    reset = function(t)t.label='UNDEFINED';t.position={0,0.28,-1.35}end
  },{
...
local oldName,basetable,TipsyColor='','0123456789abcdefghijklmnopqrstuvwxyz',nil
function tonum(num,base)
