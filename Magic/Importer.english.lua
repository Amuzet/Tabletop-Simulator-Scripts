...
local TBL = {
  ...
  __index = function(t, k)
    if type(t.___) == 'table' then rawset(t, k, t.___()) else rawset(t, k, t.___) end
    return t[k]
  end
}
---Constructs a table with a default value for all keys.
---@param d any Default value
...
---Table for spawning information
---@alias qTbl { player: string, deck: number, image: table, position: any, color: any, text: string, full: any, url: string, mode: string }
---Mutates into qTbl
---@alias callTbl { position: any, player: string, color: string, url: string, mode: string, name: string, full: string, [any]: any }

-- List of all the spawn locations for requests
textItems = {}
-- Called when spawning cards to put a marker on the table.
...
local DeckCardIndex, TickConstant, isTest, CardQualityPerUser, BackImagesPerUser = 1, 0.2, false, TBL.new('normal', {}), TBL.new('https://i.stack.imgur.com/787gj.png', {})

--Image Handler
---@param uri string
---@param quality `normal`
---@param suffix string
function truncateURI(uri, quality, suffix)
  if quality == 'png' then uri = uri:gsub('.jpg', '.png') end
...
local Card = setmetatable({ n = 1, image = false },
  {
    ---@param this any
    ---@param cardInformation any
    ---@param qTbl qTbl
    __call = function(this, cardInformation, qTbl)
      success, errorMSG = pcall(function()
        --NeededFeilds in c:name,type_line,cmc,card_faces,oracle_text,power,toughness,loyalty
        cardInformation.face, cardInformation.oracle, cardInformation.back = '', '', BackImagesPerUser[qTbl.player] or BackImagesPerUser.___
        local n, state, qual, imgSuffix = this.n, false, CardQualityPerUser[qTbl.player], ''
        this.n = n + 1
        --Check for card's spoiler image quality
        if cardInformation.image_status ~= 'highres_scan' then
        ...
        if cardInformation.card_faces and cardInformation.image_uris then --Adventure/Split
          local instantSorcery = 0
          for i, f in ipairs(cardInformation.card_faces) do
            f.name = f.name:gsub('"', '') .. '\n' .. f.type_line .. '\n' .. cardInformation.cmc .. 'CMC'
            if i == 1 then cardInformation.name = f.name end
            cardInformation.oracle = cardInformation.oracle .. f.name .. '\n' .. setOracle(f) .. (i == #cardInformation.card_faces and '' or '\n')

            --Count nonPermanent text boxes, exclude Aftermath
            if not cardInformation.oracle:find('Aftermath') and
        ...
        elseif cardInformation.card_faces then --DFC
          ...
          cardInformation.name = cardInformation.name:gsub('"', '') .. '\n' .. cardInformation.type_line .. '\n' .. cardInformation.cmc .. 'CMC'
          cardInformation.oracle = setOracle(cardInformation)
          if ('planar'):find(cardInformation.layout) then orientation[1] = true end
        end

        local backDat = nil
        --Image Handling
        if qTbl.deck and qTbl.image and qTbl.image[n] then
          cardInformation.face = qTbl.image[n]
        elseif cardInformation.card_faces and not cardInformation.image_uris then --DFC REWORKED for STATES!
          local faceAddress = truncateURI(cardInformation.card_faces[1].image_uris.normal, qual, imgSuffix)
          local backAddress = truncateURI(cardInformation.card_faces[2].image_uris.normal, qual, imgSuffix)
          ...
          backDat = {
            ...
            Nickname = name,
            Description = oracle,
            Memo = cardInformation.oracle_id,
            CardID = b * 100,
            CustomDeck = {
              [b] = {
                FaceURL = backAddress,
                BackURL = cardInformation.back,
                NumWidth = 1,
                NumHeight = 1,
                Type = 0,
                BackIsHidden = true,
        ...
        elseif this.image then --Custom Image
          cardInformation.face = this.image
          this.image = false
        elseif cardInformation.image_uris then
          cardInformation.face = truncateURI(cardInformation.image_uris.normal, qual, imgSuffix)
        end

        -- One single card's data for spawnObjectData
        local cardDat = {
          Transform = { posX = 0, posY = 0, posZ = 0, rotX = 0, rotY = 0, rotZ = 0, scaleX = 1, scaleY = 1, scaleZ = 1 },
          Name = "Card",
          Nickname = cardInformation.name,
          Description = cardInformation.oracle,
          Memo = cardInformation.oracle_id,
          CardID = n * 100,
          CustomDeck = {
            [n] = {
              FaceURL = cardInformation.face,
              BackURL = cardInformation.back,
              NumWidth = 1,
              NumHeight = 1,
              Type = 0,
        ...
        if not (qTbl.deck) or qTbl.deck == 1 then
          ...
          uLog(qTbl.color .. ' spawned ' .. cardInformation.name:gsub('\n.*', ''))
          endLoop()
        else                         --Spawn deck
          if DeckCardIndex == 1 then --initialize deckDat
            deckDat = {}
            deckDat = {
              Transform = { posX = 0, posY = 0, posZ = 0, rotX = 0, rotY = 0, rotZ = 0, scaleX = 1, scaleY = 1, scaleZ = 1 },
              Name = "Deck",
          ...
          deckDat.DeckIDs[DeckCardIndex] = cardDat.CardID -- add card info into deckDat
          deckDat.CustomDeck[n] = cardDat.CustomDeck[n]
          deckDat.ContainedObjects[DeckCardIndex] = cardDat
          if DeckCardIndex < qTbl.deck then
            qTbl.text('Spawning here\n' .. DeckCardIndex .. ' cards loaded')
            DeckCardIndex = DeckCardIndex + 1
          elseif DeckCardIndex == qTbl.deck then
            local spawnDat = {
              data = deckDat,
              position = qTbl.position or { 0, 2, 0 },
            ...
            spawnObjectData(spawnDat)
            Player[qTbl.color].broadcast('All ' .. DeckCardIndex .. ' cards loaded!', { 0.5, 0.5, 0.5 })
            DeckCardIndex = 1
            endLoop()
          end
        end
      ...
      if not success then
        ...
        for _, o in ipairs(textItems) do
          if o ~= nil then
            o.destruct()
          end
        end
...
---@param cardInformation any
---@return string
function setOracle(cardInformation)
  local n = '\n[b]'
  if cardInformation.power then
    n = n .. cardInformation.power .. '/' .. cardInformation.toughness
  elseif cardInformation.loyalty then
    n = n .. tostring(cardInformation.loyalty)
  else
    n = ''
  end
  return cardInformation.oracle_text:gsub('\"', "'") .. (#n > 0 and n .. '[/b]' or '')
end

---@param webRequest any
---@param qTbl qTbl
---@param originalData any
function setCard(webRequest, qTbl, originalData)
  if webRequest.text then
    local jsonCardInformation = JSON.decode(webRequest.text)

    if jsonCardInformation.object == 'card' then
      --Fancy Art Series
      if originalData and originalData.layout == 'art_series' then
        for k in ('mana_cost type_line oracle_text colors power toughness loyalty'):gmatch('%S+') do
          ...
          for i = 1, 2 do
            if jsonCardInformation.card_faces and jsonCardInformation.card_faces[i][k] then
              originalData.card_faces[i][k] = jsonCardInformation.card_faces[i][k]
            elseif jsonCardInformation[k] then
              originalData.card_faces[i][k] = jsonCardInformation[k]
            end
          end
        end
        for k in ('cmc type_line color_identity layout'):gmatch('%S+') do
          if jsonCardInformation[k] then
            originalData[k] = jsonCardInformation[k]
          end
        end
        if jsonCardInformation.image_uris then
          originalData.card_faces[2].image_uris = jsonCardInformation.image_uris
        else
      ...
      elseif jsonCardInformation.layout == 'art_series' then
        WebRequrest.get('http://api.scryfall.com/cards/named?fuzzy=' .. jsonCardInformation.card_faces[1].name, function(request)
          ...
          if locale_json.object == 'error' then
            ...
            setCard(request, qTbl, jsonCardInformation)
          end
        end)
...
---@param webRequest any
---@param qTbl qTbl
function spawnList(webRequest, qTbl)
  uLog(webRequest.url)
  local txt = webRequest.text
  if txt then --PIE's Rework
    ...
    if jsonType == 'list' then
      ...
      if nCards ~= nil then
        ...
        if nCards == nil then
          nCards = 0
          printToAll('Tell Amuzet that an Error occored in spawnList: nCards')
        end
      else
      ...
      for i = 1, nCards do
        ...
        Wait.time(function() Card(card, qTbl) end, i * TickConstant)
      end
      return
    elseif jsonType == 'card' then
      local _, json = 1, JSON.decode(txt)
      Card(json, qTbl)
      return
...
---@param webRequest any
---@param qTbl callTbl
function spawnDeck(webRequest, qTbl)
  if webRequest.text:find('!DOCTYPE') then
    uLog(webRequest.url, 'Mal Formated Deck ' .. qTbl.color)
    uNotebook('D' .. qTbl.color, webRequest.url)
    Player[qTbl.color].broadcast('Your Deck list could not be found\nMake sure the Deck is set to PUBLIC', { 1, 0.5, 0 })
    textItems[#textItems].destruct()
    table.remove(textItems, #textItems)
  else
    uLog(webRequest.url, 'Deck Spawned by ' .. qTbl.color)
    local sideboard = ''
    qTbl.image = {}
    local deck, list = {}, webRequest.text:gsub('\n%S*Sideboard(.*)', function(a)
      sideboard = a
      return ''
    end)
    if sideboard ~= '' then
...
---@param webRequest any
---@param qTbl qTbl
function spawnCSV(webRequest, qTbl)
  local side, deck, list = '', {}, webRequest.text
  for line in list:gmatch('([^\r\n]+)') do
...
local DeckSites = {
  ...
  tappedout = function(a)
    if a:find('/lists/') then setCSV = 3 else setCSV = 4 end
  ...
  archidekt = function(a)
    return 'https://archidekt.com/api/decks/' .. a:match('/(%d+)') .. '/small/?format=json', function(wr, qTbl)
      ...
      if board ~= '' then
        Player[qTbl.color].broadcast(json.name .. ' Sideboard and Maybeboard in notebook.\nType "Scryfall deck" to spawn it now.')
        uNotebook(json.name, board)
      end
    end
  end,
...
function typeCo(p, t)
  ...
  for i = 13, #p do if n == i then p[i] = p[i] .. '+' .. t else p[i] = p[i] .. '+-(' .. t .. ')' end end
  return p
end

--ReplacementSlot
---@param p qTbl qTbl
---@param s string Set code
---@param a string Anti-match query suffix
---@param b string Match query suffix
function rSlot(p, s, a, b)
  for i, v in pairs(p) do if i ~= 6 then p[i] = v .. a else p[i] = apiSet .. s .. '+' .. rarity() .. b end end
  return p
end

---@param p qTbl qTbl
---@param s string Set code
function commanderBooster(p, s)            --wubrg CCCCC CCUUU URLLF
...
---@param p qTbl qTbl
---@param s string Set code
function doubleRareBooster(p, s)
  p[11] = p[12]; table.insert(p, apiSet .. s .. '+' .. rarity(8, 1))
  return p
end

---@param p qTbl qTbl
---@param s string Set code
function dfcBooster(p, s) return rSlot(p, s, '+-is:transform', '+is:transform') end

---@param p qTbl qTbl
---@param s string Set code
function midvowBooster(p, s)
  local n = math.random(#p - 1, #p)
  for i, v in pairs(p) do if i == 6 or i == n then p[i] = p[i] .. '+is:transform' else p[i] = p[i] .. '+-is:transform' end end
  return p
end

---@param p qTbl qTbl
---@param s string Set code
function conspiracyBooster(p, s) return rSlot(p, s, '+-wm:conspiracy', '+wm:conspiracy') end

---@param p qTbl qTbl
---@param s string Set code
function ravnicaBooster(p, s) return rSlot(p, s, '+-t:land', '+t:land+-t:basic') end

---@param p qTbl qTbl
---@param s string Set code
function snowBooster(p, s)
...
local Booster = setmetatable({
  ...
  ADAMS = function(qTbl)
    ...
    for _ = 1, 5 do table.insert(pack, u .. 'r:common+-t:basic') end
    for _ = 1, 3 do table.insert(pack, u .. 'r:uncommon') end
  ...
  CONSPIRACY = function(qTbl) --wubrgCCCCCTUUURT
    local p = self('(s:cns+or+s:cn2)')
  ...
  INNISTRAD = function(qTbl) --wubrgDCCCCDUUURD
    local p = self('(s:isd+or+s:dka+or+s:avr+or+s:soi+or+s:emn+or+s:mid+s:vow)')
  ...
  RAVNICA = function(qTbl) --wubrgmmm???UUURL
    local l, p = 't:land+-t:basic', self('(s:rav+or+s:gpt+or+s:dis+or+s:rtr+or+s:gtc+or+s:dgm+or+s:grn+or+s:rna)')
    table.insert(p, p[#p])
  ...
  KAMIGAWA = function(qTbl) --wubrgCCCCCCUUURN
    local p = self('(s:chk+or+s:bok+or+s:sok+or+s:neo)')
  ...
  MIRRODIN = function(qTbl)
    local p = self('(s:mrd+or+s:dst+or+s:5dn+or+s:som+or+s:mbs+or+s:nph)')
    return p
  end,
  PHYREXIA = function(qTbl)
    local p = self.MIRRODIN(qTbl)
    table.insert(p, p[#p])
    p[11] = p[12]
  ...
  ZENDIKAR = function(qTbl)
    local p = self('(s:zen+or+s:wwk+or+s:roe+or+s:bfz+or+s:ogw+or+s:znr)')
    --Masterpiece
  ...
  ['clb'] = commanderBooster,
  ['cmr'] = commanderBooster,
  ['2xm'] = doubleRareBooster,
  ['2x2'] = doubleRareBooster,
  ['isd'] = dfcBooster,
  ['dka'] = dfcBooster,
  ['soi'] = dfcBooster,
  ['emn'] = dfcBooster,
  ['cns'] = conspiracyBooster,
  ['cn2'] = conspiracyBooster,
  ['rav'] = ravnicaBooster,
  ['gpt'] = ravnicaBooster,
  ['dis'] = ravnicaBooster,
  ['rtr'] = ravnicaBooster,
  ...
  __call = function(t, set, n)
    ...
    if not n and t[set] and type(t[set]) == 'function' then
      ...
      for _ = 1, 6 do table.insert(pack, u .. 'r:common+-t:basic') end
...
Importer = setmetatable({
  ...
  Back = function(qTbl)
    ...
    BackImagesPerUser[qTbl.player] = qTbl.url
      ...
      if json.all_parts then
        ...
        for _, v in ipairs(json.all_parts) do
          if json.name ~= v.name then
            WebRequest.get(v.uri, function(wr) setCard(wr, qTbl) end)
          end
        end
  ...
  Quality = function(qTbl)
    if ('small normal large art_crop border_crop'):find(qTbl.name) then
      CardQualityPerUser[qTbl.player] = qTbl.name
    end
    endLoop()
  end,
  --[[Lang]]
  ...
  Lang = function(qTbl)
    ...
    if lang and lang ~= '' then
      printToColor('Change the language to ' .. lang, { 0.9, 0.9, 0.9 }, qTbl.color)
      return false
    else
      printToColor('Please type specific language', { 0.9, 0.9, 0.9 }, qTbl.color)
      return false
    end
    -- endLoop()
  end,
  ...
  ---@param this any self
  ---@param qTbl qTbl Table for the spawning information
  __call = function(this, qTbl)
    ...
    elseif qTbl and this.request[2] then
      ...
      elseif this.request[3] then
        msg = msg .. '. Type `Scryfall clear queue` to Force quit the queue!'
      end
    ...
    elseif qTbl then
      broadcastToAll('Something went Wrong please contact Amuzet\nImporter did not get a mode. MAIN LOGIC')
    end
  end
})
-- Why does this exist? It's not used anywhere.
MODES = ''
for k, _ in pairs(Importer) do
...
-- Help text posted in chat
local UsageHelpMessage = [[    [b]%s
...
---@param fN function
---@param tbl number | table
function delay(fN, tbl)
  local timerParams = { function_name = fN, identifier = fN .. 'Timer' }
  if type(tbl) == 'table' then timerParams.parameters = tbl end
  if type(tbl) == 'number' then
    timerParams.delay = tbl * TickConstant
  else
...
---@param a string
---@param b string
function uLog(a, b) if isTest then log(a, b) end end
...
function uNotebook(t, b, c)
  ...
  if p.index < 0 then addNotebookTab(p) else editNotebookTab(p) end
  return p.index
end
...
--[[Tabletop Callbacks]]
function onSave() self.script_state = JSON.encode(BackImagesPerUser) end
...
function onLoad(data)
  ...
  if data ~= '' then BackImagesPerUser = JSON.decode(data) end
  BackImagesPerUser = TBL.new('https://gamepedia.cursecdn.com/mtgsalvation_gamepedia/f/f8/Magic_card_back.jpg', BackImagesPerUser)
...
function onPlayerConnect(p)
  if p.steam_id == author then
    AP('Welcome %s, creator of me. The Card Importer!')
  elseif p.steam_id == coauthor then
    AP('Praise be to %s!')
  end
end
...
function onPlayerDisconnect(p)
  if p.steam_id == author then
    AP('Goodbye %s, take care of yur self buddy-o-pal!')
  elseif p.steam_id == coauthor then
    AP('𝜋 doesn\'t terminate, but %s does.')
  end
end

-- Toggles supressing or showing spawn messages in chat.
local chatToggle = false
---@param message string
---@param player any
function onChat(message, player)
  if message:find('!?[Ss]cryfall ') then
    local arguments = message:match('!?[Ss]cryfall (.*)') or false
    if arguments == 'hide' and player.admin then
      chatToggle = not chatToggle
      if chatToggle then message = 'supressing' else message = 'showing' end
      broadcastToAll('Importer now ' .. message .. ' Chat messages with Importer in them.\nToggle this with "Scryfall Hide"', SMC)
    elseif arguments == 'help' then
      player.print(UsageHelpMessage, { 0.9, 0.9, 0.9 })
      return false
    elseif arguments == 'promote me' and player.steam_id == author then
      player.promote()
    elseif arguments == 'clear queue' then
      version = version - 1
      printToAll(SMG .. 'Respawning Importer!', SMC)
      self.reload()
    elseif arguments == 'clear back' then
...
"76561198317076000":"https://i.imgur.com/vh8IeEn.jpeg"}]], '\n', '')
      BackImagesPerUser = TBL.new('https://gamepedia.cursecdn.com/mtgsalvation_gamepedia/f/f8/Magic_card_back.jpg', JSON.decode(self.script_state))
    ...
    elseif arguments then
      ...
      local tbl = {
        ...
        color = player.color,
        url = arguments:match('(http%S+)'),
        ---@type string Any of the callable modes in the Importer table.
        mode = arguments:gsub('(http%S+)', ''):match('(%S+)'),
        name = arguments:gsub('(http%S+)', ''),
        full = arguments
      }
      ...
      -- Execute the function associated with the mode
      Importer(tbl)
      if chatToggle then
        uLog(message, player.steam_name)
        return false
      end
    end
  end
...
function toggleMenu(o)
  ...
  if enc then
    ...
    for _, v in ipairs(buttons) do Button(o, v, flip) end
    Button:reset()
  end
end
...
Button = setmetatable({
  ...
  rotation = { 0, 0, 90 },
  reset = function(t)
    t.label = 'UNDEFINED'; t.position = { 0, 0.28, -1.35 }
  end
}, {
