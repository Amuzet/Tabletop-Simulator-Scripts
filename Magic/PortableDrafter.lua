...
local Card = setmetatable({
    ...
    __call = function(t,c)
      --Error with Mirrage, BETA: Attempt to index a nil value
      assert(c,'No Card Passed to Card()')
      if c.name then
        t.n = t.n + 1
        t.json = string.format(
          ...
          c.name , c.oracle , t.n , t.n , c.face , Back)
        spawnObjectJSON(t)
      end
    end
  })
...
local Button = setmetatable({
    ...
    __call = function(t)
      ...
      if Cards[TAG] then else
        broadcastToAll('[dc143c]Warning: [4682b4]Server will stutter loading [-]'..Count..' Cards')
      end
    ...
    if alt then
      ...
      function initCard(wr)
        ...
        if card.image_uris then
          tbl.face = card.image_uris.normal:gsub('%?.*','')
        else
          tbl.face = card.card_faces[1].image_uris.normal:gsub('%?.*','')
        end
        ...
        --  tbl.rarity = 'TOKENS'
          ...
          if #card.colors == 1 and tbl.rarity == 'COMMON' then
        ...
        if slot then
          if Cards[TAG][slot] then
            table.insert( Cards[TAG][slot], tbl )
          else
            Cards[TAG][slot] = { tbl }
          end
        end
        if Cards[TAG][tbl.rarity] then
          table.insert( Cards[TAG][tbl.rarity], tbl )
        else
          Cards[TAG][tbl.rarity] = { tbl }
        end
      end
      function setLoad(tag,count)
        ...
        for i=1, count do
          Wait.time(function()
              ...
              WebRequest.get(url,function(Scryfall_Card) initCard(Scryfall_Card) end)
            end,i*Tic)
        end
      end
      if Cards[TAG] then
        ...
        Cards[TAG] = {W=0,U=0,B=0,R=0,G=0}
        setLoad(TAG,Count)
      end
      Wait.time(function()
          if sGen[TAG] and Count ~= 0 then
            local s=sGen[TAG]
            setLoad(s[1],s[2])
...
function makeBoosters(a)
  ...
  pos = {
    ...
    pos[2]+0.5,
    pos[3]-3.4}
  if Cards[TAG]['BASICS'] then
    Card.position = {pos[1]-3.6,pos[2],pos[3]-3.4}
    for k,c in pairs( Cards[TAG]['BASICS'] ) do
      Card(c)
    end
  end
  
  local pack = {}
  if sPak[TAG] then pack = sPak[TAG] end
  for k,v in pairs({COMMON=11,UNCOMMON=3,RARE=1,MYTHIC=8}) do
    if pack[k] == nil then
      pack[k] = v
    end
  end
  ...
  for i=0, n-1 do
    Wait.time(function()
        local p=pack
        createBooster({pos[1]+2.4*(i%4),pos[2],pos[3]+3.4*math.floor(i/4)},p)
      end,i)
  end
end
...
function createBooster( pos, pack )
  ...
  elseif not Cards[TAG]['MYTHIC'] then
    pack.MYTHIC = 0
  end
  --Double Faced / Conspiracy
  if pack.EXTRA then
    local eS = math.random(1,pack.EXTRA[4]+pack.EXTRA[3]+pack.EXTRA[2]+pack.EXTRA[1])
    if eS > pack.EXTRA[3]+pack.EXTRA[2]+pack.EXTRA[1] then
      pack.EXTRACOMMON = 1
    elseif eS > pack.EXTRA[2]+pack.EXTRA[1] then
      pack.EXTRAUNCOMMON = 1
    elseif eS > pack.EXTRA[1] then
      pack.EXTRARARE = 1
    else
      pack.EXTRAMYTHIC = 1
    end
  ...
  elseif pack.PLANESWALKER then perPackSlot('PLANESWALKER')
  end
  
  local genNums,common = {},pack.COMMON
  
  for r,n in pairs( pack ) do
    if not Cards[TAG][r] then
      --log('Rarity Not Found')
    ...
    elseif n > 0 then
      ...
      genNums = setmetatable({
          },{
          __call = function(t,num)
      ...
      if r == 'COMMON' then
        ...
        for _,k in ipairs({'W','U','B','R','G'}) do
          --Evaluate where the last card of that color is
          ...
          if Cards[TAG][r][d] then
            ...
            printToAll('Common Not Found '..d..'/'..#Cards[TAG][r],{1,0,1})
          end
      ...
      for i=1,pack[r] do
        ...
        while genNums(rng) do
          rng,L = math.random(1,#Cards[TAG][r]),L+1
        end
      ...
      --Log Pack Generation
      local bool = false
      for k,m in ipairs(genNums) do
        if genNums(m) and genNums(m) ~= k and type(m) == 'number' then
          bool = true end end
      if bool then
        local asdf1 = '['..string_to_color(r)..']'..r
        for i,v in ipairs(genNums) do
          asdf1 = asdf1..'_'..v end
        print(asdf1..'[-]') end
    end
  end
...
function versionCheck(wr)
  ...
  if version<tonumber(v)then
    txt=txt..'\n[fff600]Consider Updating to '..mod_name..' '..v
  end
