--By Amuzet
mod_name,version='Planesculptor',0.1
...
for _,s in pairs({'ice','mh1'})do Booster[s]=function(p)p[math.random(6,11)]=apiSet..s..'+t:basic+t:snow'return p end end
for _,s in pairs({'rav+t:land+-t:basic','gpt+t:land+-t:basic','dis+t:land+-t:basic','rtr+t:land+-t:basic','gtc+t:land+-t:basic','dgm+t:land+-t:basic','grn+t:land+-t:basic','rna+t:land+-t:basic',
    'cns+wm:conspiracy','cn2+wm:conspiracy'})do
  local k=s:match('%w+');Booster[k]=function(p)p[math.random(6,11)]=apiSet..s..rarity();return p end end
...
local Importer=setmetatable({
  ...
  Spawn=function(qTbl)
    WebRequest.get('http://www.planesculptors.net/explore?query='..qTbl.name,function(a)
        local short=a.text:gsub('.*<p>Found ','',1)
        local n=tonumber(short:match('%d+'))
        if not n or n==0 then
          Player[qTbl.color].broadcast('Could not find '..qTbl.name,{1,0,1})endLoop()end
        local i=0
        for w in short:gmatch('<a href="(/set/[^"]*)" title="')do i=i+1
          Wait.time(function()WebRequest.get('http://www.planesculptors.net'..w,function(wr)
                local img,txt=wr.text:match('(http://www.planesculptors.net/upload/[^\']+)'),wr.text:match('<textarea>(.*)</textarea>')
                
                
                end)end,i*Tick*2)end end)end,
...
function onChat(msg,player)
  if msg:find('[Pp]lanesculpt ')or msg:find('!P%S* ')then
    local a=msg:match('[Pp]lanesculpt (.*)')or msg:match('!P%S* (.*)')or false
    if a=='hide'and player.admin then
      chatToggle=not chatToggle
      if chatToggle then msg='supressing' else msg='showing'end
      broadcastToAll('Planesculptor now '..msg..' Chat messages with Importer in them.\nToggle this with "Importer Hide"',{0.9,0.6,0.4})
    elseif a=='help'then
